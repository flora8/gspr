...
def Survey(): # Collecting user inputs for later analysis
    ...
    with col1:
        ...
        EMDN_category = st.selectbox("Which EMDN code category of medical device are you particularly interested in searching for on this application?", list(emdn_E_part)) # set index to none means there is no default options
        ...
        EMDN_type = st.selectbox("Which EMDN code type of medical device are you particularly interested in searching for on this application?", list(group_E.iloc[:,0]))
        EMDN_category_all = st.selectbox("Which EMDN code category of medical device are you interested in searching for in the future?", list(emdn_E_all)) # set index to none means there is no default options
        group_E_all = emdn_E_all.groupby(by=[EMDN_category_all], as_index=False)[[]].sum() # Group the EMDN code type based on the specific category chosen
        EMDN_type_all = st.selectbox("Which EMDN code type of medical device are you interested in searching for in the future?", list(group_E_all.iloc[:,0]))
    ...
    with col2:
        ...
        EMDN_category_C = st.selectbox("應用程式顯示的資訊，請問您對哪種 EMDN 分類的醫療器材特別感興趣搜尋?", list(emdn_C_part)) # set index to none means there is no default options
