...
def Home():
    ...
    with col2:
        ...
        st.markdown("""
                    ...
                    **邀請段落：**  本人誠摯地邀請您參與這個研究項目，這是我碩士學位研究的一部分。在您決定是否參與之前，了解研究的目的及參與的內容是非常重要的，請您仔細閱讀以下信息，感謝。
    
                    **研究目的？**  非常感謝您抽出寶貴時間參與這項碩士生論文研究。本研究旨在設計一種監管工具，以正確過濾和選擇醫療器材所需滿足的適當監管要求。具體目標是評估監管工具的可用性並收集初始使用者體驗，以了解該工具的需求並為未來開發的改進提供見解。
                    ...
                    **如果我有進一步的問題，或者出了問題怎麼辦？**  如果這項研究以任何方式對您造成了傷害，或者如果您希望對研究提出投訴，您可以使用以下詳細資訊聯繫倫敦國王學院以獲取進一步的建議和訊息：Yen-Hua Ho (yen-hua.ho@kcl.ac.uk), Dr. Clare Heaysman (clare.heaysman@kcl.ac.uk)
                    ...
                    請注意，本系統提供的所有資訊僅供參考，因法規和標準日益更新而資料庫可能不涵蓋最新消息。最後感謝您閱讀本資訊並考慮參與這項研究，煩請點擊左側的小三角符號:blue-background[>]查看下一頁並進行測試。
                    """)
...
def Survey(): # Collecting user inputs for later analysis
    ...
    with col2:
        ...
        information_C = st.selectbox("請問您對本應用網站所提供的整體醫材資訊評價如何？", ("","1: 非常適當和明確", "2: 適當和明確", "3: 普通", "4: 不適當和不明確", "5: 非常不適當和不明確"))
        experience_C = st.selectbox("請問您如何看待使用監管工具應用網站的好處？", ("","1: 非常有幫助", "2: 有幫助", "3: 普通", "4: 無幫助", "5: 非常無幫助"))
        ...
        others_C = st.text_area("請問您希望在此監管工具應用網站上看到哪些其他資訊？")
...
def Analysis(): # Plotting and data visualisation to analyse user experience survey result
    ...
    with Counts: # User select the x-axis to plot the counts
        ...
        count_E.columns = ["index", "count"] 
        fig2_E = px.pie(count_E, values="count", names="index", title="Pie chart: {} distribution".format(xvalue_E)) # Display the distribution of species in the data
    ...
    with 數量: # User select the x-axis to plot the counts  
        ...
        count_C.columns = ["index", "count"] 
        fig2_C = px.pie(count_C, values="count", names="index", title="圓餅圖: {}分佈".format(xvalue_C)) # Display the distribution of species in the data
