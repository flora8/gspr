...
def Analysis(): # Plotting and data visualisation to analyse user experience survey result
    ...
    with Counts: # User select the x-axis to plot the counts
        ...
        count2_E = data_E[[xvalue_E]].groupby(by=xvalue_E)[xvalue_E].sum()
    ...
    with Analysis: # User select the x-axis and y-axis value to plot the analysis data
        ...
        plot_E = px.scatter(data_E, x=xaxis_E, y=yaxis_E, size="petal_length", hover_data=['petal_width'], labels={xaxis_E:yaxis_E}, title="The searched {} by {} results".format(xaxis_E,yaxis_E))
    ...
    with 分析: # User select the x-axis and y-axis value to plot the analysis data
        ...
        plot_C = px.scatter(data_C, x=xaxis_C, y=yaxis_C, size="petal_length", hover_data=['petal_width'], labels={xaxis_C:yaxis_C}, title="依照 {} 搜尋 {} 的結果".format(xaxis_C,yaxis_C))
